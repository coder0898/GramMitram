...
const Dashboard = () => {
  ...
  const { cards, pieData, barData } = useMemo(() => {
    ...
    if (role === "staff") {
      ...
      const cards = [
        ...
        {
          label: "Approved",
          value: staffStats.approved,
          color: "success.main",
        },
        { label: "Rejected", value: staffStats.rejected, color: "error.main" },
      ];

      const pieData = [
        { name: "Under Review", value: staffStats.underReview },
        { name: "Approved", value: staffStats.approved },
        { name: "Rejected", value: staffStats.rejected },
      ];
  ...
  const COLORS_PIE = ["#1976d2", "#ff9800", "#4caf50", "#f44336"];
  const COLORS_ADMIN_PIE = ["#FFBB28", "#00C49F", "#FF8042"];
  const COLORS_STAFF_PIE = ["#FFBB28", "#4caf50", "#f44336"]; // under review, approved, rejected
